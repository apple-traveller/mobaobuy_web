...
<script type="text/javascript">
                ...
                success:function(res){
                	var result = JSON.parse(res);
                   	 if(result['code']){
                   		var result = JSON.parse(res);
                   	 	var str = '';
                   	 	str = '<div><p>用户名：'+result['info']['user_name']+'</p><p>昵称：'+result['info']['nick_name']+'</p><input id="po" type="checkbox" name="can" value="1" /> 采购<input type="checkbox" id="pay" name="can" value="2"/> 付款　<input type="hidden" name="_id" value="'+result['info']['id']+'"><input type="button" name="" value="确认" onclick="permiss();"></div>';	
                   	 	$('#endsearch').html(str);
                   	 }
                }
		})
	...
	function permiss(){
		...
		$.ajax({
                headers:{ 'X-CSRF-TOKEN': $('input[name="_token"]').val()},
                'type':'post',
                'data':{'user_id':userId,'permi':permi},
                'url':'{{url('/addFirmUser')}}',
                success:function(res){
                	var result = JSON.parse(res);
                	if(result.code){
