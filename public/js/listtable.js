...
{
    $.msg.error('Ajax object doesn\'t exists.');
}

if (typeof Utils != 'object')
{
    $.msg.error('Utils object doesn\'t exists.');
}
...
/**
 * 创建一个可编辑区
 * {code:1, msg:'', data:"返回后显示的内容"}
 */
listTable.edit = function(obj, url, id, ext_params)
{
    ...
    txt.onblur = function(e)
    {
        ...
        {
            res = Ajax.call(url, "val=" + encodeURIComponent(Utils.trim(txt.value)) + "&id=" +id + (Utils.isEmpty(ext_params)?'':("&" + ext_params)), null, "POST", "JSON", false);
            ...
            obj.innerHTML = (res.code == 1) ? res.data : org;
...
/* 按钮切换 */
listTable.switchBt = function(obj, url, id, ext_params)
{
    var obj = $(obj);
    var val = (obj.attr('class').match(/active/i)) ? 0 : 1;
    var res = Ajax.call(url, "val=" + val + "&id=" +id+ (Utils.isEmpty(ext_params)?'':("&" + ext_params)), null, "POST", "JSON", false);
 ...
 * 删除列表中的一个记录
 */
listTable.remove = function(id, cfm, url, callback)
{
    if (confirm(cfm))
    {
        var args = "id=" + id;
        Ajax.call(url, args, callback, "GET", "JSON");
    }
}
/**
 * 列表全选和取消全选
