...
$(function(){
		...
		function db_column(){
			...
			$(document).on('click','*[ectype="delete_adv"]',function(){
				...
				Ajax.call('visualhome.php', "act=delete_adv&suffix=" +  suffix, function(data){
					_this.parents('.page-head-bgimg').find('img').attr('src',"/default/visualDefault/bgimg.gif");
	...
	function visualOperation(){
		...
		downloadModal.on("click",function(){
			var Release_url = '';
			var Release_where = '';
            Release_url = "/template/publish";
            var content = $(".pc-page").html();

			if(confirm("确定发布？")){
				$.post(Release_url, {"_token":$("#_token").val(),"content":content}, function(data){
					alert("发布成功");
					$("[ectype='back']").hide();
				}, 'JSON');
			}
	...
	//可视化区域编辑
/*
<div class="topBanner lyrow ui-draggable ui-box-display"
data-purebox="banner"
data-mode="topBanner"
ectype="visualItme"
data-diff="0"
...
*/
	...
	visualShell.on("click","*[ectype='model_edit']",function(){
		...
		switch(purebox){
            case "cust":
				...
				$.post("/template/partEdit",{"_token":$("#_token").val(),"mode":mode,"spec_attr":encodeURIComponent(spec_attr),"masterTitle":escape(masterTitle),"lift":lift,"diff":diff,"hierarchy":hierarchy,"suffix":suffix},function(res){
                    if(res.code=200){
                        layui.use(['layer','laypage'], function(){
                            ...
                            for(var i=0;i<7;i++){
                                html+='<li class="addpic" data_src="/default/1/'+arr[i]+'"><img src="/default/1/'+arr[i]+'"><span class="pixel">232x280</span></li>'
                            }

                            var content = ' <div class="pb-bd">\n' +
                                '            <div class="pb-ct">\n' +
                                '                <div class="tishi">\n' +
                            ...
                            getpics(content,1,7,laypage,res);

                            $(document).delegate(".pb-btn","click",function(){
                                layer.close(index);
                                var ggpic = $(".imgInput .img span img").attr("src");
                                console.log(ggpic);
								$(".advPictures").attr('src',ggpic);
                            });
				...
				//Ajax.call('/template/partEdit', "act=home_adv&mode=" + mode + '&spec_attr=' + encodeURIComponent(spec_attr) + "&masterTitle=" + escape(masterTitle) + "&lift=" + lift + "&diff=" + diff + "&hierarchy=" + hierarchy + "&suffix=" + suffix,dialogResponse, 'POST', 'JSON');
			break; 
			
			case "homeFloor":
                $.ajaxSettings.async = false;
                $.post("/template/partEdit",{"_token":$("#_token").val(),"mode":mode},function(res){
                    if(res.code=200){
                        layui.use(['layer','laypage'], function(){
                            var layer = layui.layer;
                            var laypage = layui.laypage;
    ...
    $(document).delegate(".addpic","click",function(){
        $(".imgInput").html();
		...
		'                  <input type="hidden" name="pic_src[]" value="'+src+'">\n' +
		'                </div>\n' +
		'                </td>\n' +
		'                <td>\n' +
		'                      <input type="text" name="link[]" value="" class="form-control">\n' +
		...
		'                 </td>\n' +
		'              </tr>';
        var img = $(".imgInput").html(html);
    });

    //ajax分页
    function getpics(content,curr,pagesize,laypage,res){
        ...
        laypage.render({
            ...
            ,jump: function(obj, first){
                if(!first){
                    ...
                    var arr = res.data.list;
                    $("#layer").html();
                    for(var i=(curr-1)*pagesize;i<(curr-1)*pagesize+pagesize;i++){
                        html+='<li class="addpic" data_src="/default/1/'+arr[i]+'"><img src="/default/1/'+arr[i]+'"><span class="pixel">232x280</span></li>'
                    }
                    var content = ' <div class="pb-bd">\n' +
                        '            <div class="pb-ct">\n' +
                    ...
                    $("#layer").html(content);
                    getpics(content,curr,pagesize,laypage,res);
                }
            }
        });
    ...
    //         var content = $(".pc-page").html();
    //
    //         $.post('/template/saveTemplate',{"content":content},function(res){
    //             if(res.code==200){
    //                 //alert("123");
    //                 layer.msg(res.msg,{time:500,offset:["50%","50%"]});
    //
    //             }else{
    //                 layer.msg(res.msg,{time:500,offset:["50%","50%"]});
    //             }
