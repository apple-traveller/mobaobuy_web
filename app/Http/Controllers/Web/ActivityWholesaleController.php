...
//        $condition['end_time|>'] = Carbon::now();
        try{
            $promoteInfo = ActivityWholesaleService::buyLimit($condition);
    ...
    public function buyLimitDetails($id){
        ...
        //进入详情页 增加点击量
        try{
            $res = ActivityWholesaleService::addClickCount($id);
            $goodsInfo = ActivityWholesaleService::buyLimitDetails($id,$userId);
    ...
    public function buyLimitToBalance(Request $request){
        ...
        try{
            $activityInfo = ActivityWholesaleService::buyLimitToBalance($goodsId,$activityId,$goodsNum,$userInfo['id']);
    ...
    public function buyLimitMaxLimit(Request $request){
       ...
       try{
            $maxBuyNumInfo = ActivityWholesaleService::buyLimitMaxLimit($userId,$id,$goods_number);
